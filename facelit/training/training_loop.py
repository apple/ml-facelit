...
def save_image_grid(img, fname, drange, grid_size):
    ...
    if C == 6:
        PIL.Image.fromarray(img[:, :, :3], 'RGB').save(fname.replace('.png', '_specular.png'))
        PIL.Image.fromarray(img[:, :, 3:], 'RGB').save(fname.replace('.png', '_diffuse.png'))
...
def training_loop(
    ...
    label_dim = training_set.label_dim #-27 if G_kwargs['rendering_kwargs']['light_mode'] == 'diffuse-rsh' else training_set.label_dim
    common_kwargs = dict(c_dim=label_dim, img_resolution=training_set.resolution, img_channels=training_set.num_channels)
    ...
    while True:
        ...
        for phase, phase_gen_z, phase_gen_c in zip(phases, all_gen_z, all_gen_c):
            # print('tick tick tick', phase.name)
