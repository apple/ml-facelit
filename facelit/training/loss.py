    ...
    def accumulate_gradients(self, phase, real_img, real_c, gen_z, gen_c, gain, cur_nimg):
        ...
        if phase in ['Gmain', 'Gboth']:
            with torch.autograd.profiler.record_function('Gmain_forward'):
                ...
                if gen_img['normals_agg']:
                    loss_Gnormal = torch.nn.functional.l1_loss(gen_img['normals_agg'][0], gen_img['normals_agg'][1]) + \
                        torch.nn.functional.l1_loss(gen_img['normals_agg'][2], gen_img['normals_agg'][3])
                    loss_Gmain = loss_Gmain + self.normal_reg_weight * loss_Gnormal
