...
def init_dataset_kwargs(data, pose_cond, use_deca, use_light):
    use_labels = pose_cond or use_deca
    try:
...
@click.option('--gen_light_cond', help='If true, enable generator light conditioning.', metavar='BOOL',  type=bool, required=False, default=False)
@click.option('--light_mode', help='none, diffuse or full to be use with SH in rendering', metavar='STR',  type=click.Choice(['none', 'diffuse', 'full']), required=False, default='none')
...
@click.option('--normal_reg_weight',    help='weight on normal regularization', metavar='FLOAT', type=click.FloatRange(min=0), default=0, required=False, show_default=True)
...
def main(**kwargs):
    ...
    # Training set.
    c.training_set_kwargs, dataset_name = init_dataset_kwargs(data=opts.data, pose_cond=opts.gen_pose_cond, use_deca=opts.cfg == 'ffhq', use_light=opts.gen_light_cond)
    ...
    c.loss_kwargs.normal_reg_weight = opts.normal_reg_weight
